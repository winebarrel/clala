...
Client.prototype.standard_env = function() {
  ...
  env.set('cdr'       , function(x) { return x[x.length - 1]; });
  ...
  env.set('print'     , function(x) { console.log(lispstr(x)); });
    ...
    switch (x[0].toString()) {
      ...
      default:
        ...
        if (func_or_obj instanceof Function) {
          ...
          return func_or_obj[x[1]].apply(obj, x.slice(2));
        } else {
          return func_or_obj[x[1]];
        }
    }
  }
};
...
Client.prototype.lispstr = function(exp) {
  if (exp instanceof Array) {
    return '(' + exp.map(function(i) { return lispstr(i) }).join(' ') + ')';
  } else {
