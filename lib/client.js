...
Client.prototype.standard_env = function() {
  ...
  env.set('apply'     , function(f) { return f.apply(this, Array.prototype.slice.call(arguments, 1)); }.bind(this));
  ...
  env.set('procedure?', function(x) { return (x instanceof Function || x instanceof Procedure); });
  env.set('eval-js'   , function(x) { return eval(x.toString()); });
  env.set('require'   , function(x) { return require(x.toString()); });
  env.set('for-each'  , function(f, x) { x.forEach(function(i) { f.apply(this, [i]); }.bind(this)); }.bind(this));
...
Procedure.prototype.apply = function(thisArg, args) {
  return thisArg.evaluate(this.body, new Env(this.parms, args, this.env));
}
    ...
    switch (x[0].toString()) {
      ...
      default:
        ...
        if (func_or_obj instanceof Function || func_or_obj instanceof Procedure) {
          ...
          return func_or_obj.apply(this, args);
        } else if (x.length > 2) {
